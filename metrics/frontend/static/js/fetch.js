...
(async () => {
    ...
    // Fetch and load the current notice
    const notice = (await (await fetch("/v1/notice")).json()).data;
    if (!notice) return;

    const [color, text] = notice;
    const notice_element = document.getElementById("notice");

    notice_element.classList.add(`notice-${color}`);
    notice_element.innerHTML = `
        ...
        ${text}
    `;
    notice_element.style.opacity = "1";
    notice_element.style.height = "fit-content";
})();
